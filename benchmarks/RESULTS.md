...
```bash
taskset -c 2 ./build-pgo-lto-pc9/bin/sqbench --compile-repeat 3 --run-repeat 40 benchmarks/workloads/registry_catalog.nut 500
taskset -c 2 ./build-pgo-lto-pc9/bin/sqbench --compile-repeat 3 --run-repeat 40 benchmarks/workloads/world_map_graph.nut 30 18 12
taskset -c 2 ./build-pgo-lto-pc9/bin/sqbench --compile-repeat 3 --run-repeat 40 benchmarks/workloads/inventory_flow.nut 2200 11
taskset -c 2 ./build-pgo-lto-pc9/bin/sqbench --compile-repeat 3 --run-repeat 40 benchmarks/workloads/session_context_flow.nut 450 12
taskset -c 2 ./build-pgo-lto-pc9/bin/sqbench --compile-repeat 3 --run-repeat 40 benchmarks/workloads/scenario_tick_flow.nut 10200 24 14
taskset -c 2 ./build-pgo-lto-pc9/bin/sqbench --compile-repeat 3 --run-repeat 40 benchmarks/workloads/volume_presence_scan.nut 650 6 12 6
```

| Workload | run_avg_ms | checksum |
| --- | ---: | ---: |
| `registry_catalog` | `46.955` | `2019808` |
| `world_map_graph` | `48.472` | `325170` |
| `inventory_flow` | `46.322` | `580946` |
| `session_context_flow` | `41.894` | `593415` |
| `scenario_tick_flow` | `47.959` | `2030324` |
| `volume_presence_scan` | `49.901` | `308206` |

This retained head drops the refcount copy that the `_OP_PREPCALLK` cached-delegate fast path used to do (`SQObjectPtr clo = cached;`) on every cached `len` / `tointeger` / `tofloat` / `tostring` / `slice` delegate call. `clo` now starts null and is only used as the call's `retval` out-parameter; the cached native closure is passed straight to `TryFastCallNative` / `CallNative` via `_nativeclosure(cached)` (which the fast path already did for the intrinsic call), eliminating one addref+release per cached-delegate call. The host is still heavily loaded (`load avg ~34/24`), so absolute totals are elevated relative to the quieter-machine historical baselines and the authoritative signal is the live pinned PGO+LTO A/B. Correctness stayed clean (six-workload checksums matched, `samples/class.nut` / `samples/generators.nut` byte-identical to HEAD); a deterministic callgrind pass showed a broad **`-1.206%`** aggregate instruction-count reduction with every workload improving (`-0.717%` / `-1.245%` / `-1.162%` / `-1.762%` / `-1.287%` / `-1.122%`). Fresh pinned PGO+LTO A/B (candidate retrained with `train-pgo.sh`, `-DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON`) won the aggregate in both orders: candidate `46.955 / 48.472 / 46.322 / 41.894 / 47.959 / 49.901 ms` (`281.503 ms`) versus retained head `45.096 / 52.907 / 45.722 / 43.120 / 48.493 / 49.237 ms` (`284.575 ms`, `+1.07%`), then reverse candidate `45.971 / 49.446 / 46.937 / 43.689 / 48.813 / 50.740 ms` (`285.596 ms`) versus retained head `47.132 / 50.345 / 47.294 / 42.790 / 49.989 / 49.321 ms` (`286.871 ms`, `+0.44%`). The authoritative six-workload retained baseline is `281.503 ms` total on the current (loaded) host. Earlier baseline sections below are preserved for history.

### Rejected on earlier prior retained head `281.347 ms`

| Change | Frozen baseline | Retry results | Outcome |
| --- | --- | --- | --- |
| Apply the same `clo`-no-copy treatment to the `_OP_PREPCALLK` non-cached native path (`SQObjectPtr clo = temp_reg;` → `SQObjectPtr clo;` + `CallNative(_nativeclosure(temp_reg), ..., clo, ...)`), since `temp_reg` is a stable `Execute()` local and is already what the call reads | six-workload checksums still matched; `samples/class.nut` / `samples/generators.nut` byte-identical | rejected by Ir before a PGO retry: a deterministic callgrind pass went slightly net-negative at `+0.027%` aggregate (`+0.056%` / `+0.035%` / `+0.029%` / `-0.030%` / `+0.001%` / `+0.061%`) — only `session_context_flow` (heavy `.tolower()` via this path) improved, while the other workloads (which rarely reach this path) ticked up, presumably from holistic register-allocation/codegen shifts in the surrounding `_OP_PREPCALLK` handler | rolled back; unlike the cached-delegate `clo` copy (which fires on every hot delegate call), this path is cold on most workloads, so the saved addref+release is outweighed by codegen perturbation — the cached-path promotion (current retained head) already captures the broad win |
| Build with `-fno-plt` (passed via `CMAKE_CXX_FLAGS` / `CMAKE_C_FLAGS` / linker flags) to turn PLT calls into direct GOT-relative calls, on the theory that the many libc `malloc`/`memcmp`/`memcpy` calls in the hot path would shed an indirection | all six source checksums still matched; instruction-count filter is useless for a codegen flag, so a direct wall-clock A/B was used | rejected during source smoke: two pinned `--run-repeat 80` A/B pairs both regressed — control `61.796 / 61.934 / 61.256 / 53.909 / 61.420 / 62.355 ms` (`362.670 ms`) versus candidate `60.976 / 67.742 / 61.362 / 55.081 / 61.685 / 66.613 ms` (`373.459 ms`, `-2.98%`), then candidate `62.182 / 61.848 / 63.586 / 53.287 / 60.295 / 66.143 ms` (`367.341 ms`) versus control `60.890 / 62.215 / 60.727 / 56.518 / 61.743 / 63.594 ms` (`365.687 ms`, `-0.45%`) | rolled back; LTO plus the existing `-fno-semantic-interposition` already resolves internal calls directly, and the residual libc PLT calls are not a hot bottleneck, so `-fno-plt` only adds GOT-load overhead with no offsetting win |
| Build with `-march=native` (passed via `CMAKE_CXX_FLAGS`/`CMAKE_C_FLAGS`) to let GCC emit AVX-512 / BMI2 / FMA on the build host (baseline build targets plain `x86-64`); the instruction-count filter is useless here so a direct wall-clock A/B was used | all six source checksums still matched; the candidate `.so` confirmed `4` AVX-512 `zmm` instructions versus `0` in the retained head (so the flag did take effect), and the build host advertises `avx512f` / `avx2` / `bmi2` / `fma` / `popcnt` | rejected during source smoke: two pinned `--run-repeat 80` A/B pairs both regressed — control `59.695 / 61.201 / 58.824 / 55.370 / 59.806 / 63.677 ms` (`358.573 ms`) versus candidate `61.818 / 62.718 / 61.954 / 55.402 / 62.708 / 70.067 ms` (`374.667 ms`, `-4.49%`), then candidate `61.642 / 61.763 / 61.891 / 54.868 / 61.623 / 65.938 ms` (`367.725 ms`) versus control `60.384 / 60.779 / 59.862 / 61.102 / 59.510 / 63.753 ms` (`365.390 ms`, `-0.64%`) | rolled back; the workload is control-flow / refcount heavy with no SIMD-friendly inner loop, so the few emitted AVX-512 instructions only trigger Intel AVX-512 frequency downclockting (the heavy-512 license penalty) without any throughput payoff — the default `x86-64` baseline is optimal here, and lighter variants (`-mavx2`, `x86-64-v3`) were not pursued because there is no SIMD-amenable hot loop to exploit |
| Tune the thread-local small-block allocator (`sqmem.cpp`): raise `kSmallAllocMax` and/or `kSmallAllocCacheLimit` so more/larger freed blocks are reused instead of going back to libc `malloc`/`free` | all six source checksums still matched for every config tried; a deterministic callgrind sweep isolated the drivers cleanly | rejected: the small hot classes (the ones every workload shares) are the only ones worth caching deeply for `world_map_graph` — a uniform deep cache (`max=2048`,`limit=256`) cut source Ir `world_map_graph -1.311%` / aggregate `-0.239%` with no workload regressing at the source level, but a fresh pinned PGO+LTO retry lost cleanly in both orders because the deeper cache polluted the allocation-light workloads under PGO (candidate `46.964 / 48.371 / 46.996 / 42.880 / 50.201 / 51.425 ms` (`286.837 ms`) versus retained head `45.628 / 51.149 / 46.955 / 42.354 / 48.643 / 49.267 ms` (`283.996 ms`, `-1.00%`), then reverse candidate `46.680 / 49.067 / 50.939 / 44.855 / 52.170 / 50.892 ms` (`294.603 ms`) versus retained head `46.379 / 48.444 / 47.803 / 41.820 / 47.841 / 49.192 ms` (`281.479 ms`, `-4.66%`), with `world_map_graph` itself improving `-5.4%` in the first pair just as Ir predicted); a low-pollution variant (`max=4096`,`limit=64`, only newly-cached large blocks) stayed source-neutral (`-0.074%` aggregate) and a tiered variant (small `limit=64`, large `>1024` `limit=256`) went slightly net-negative (`+0.017%` aggregate), proving `world_map_graph`'s churn is in the small classes that all workloads share | rolled back; the existing `max=1024`/`limit=64` tuning is already optimal for the six-workload aggregate — `world_map_graph`'s allocation pattern conflicts with the allocation-light workloads, and no single static config wins (a workload-adaptive allocator would be needed) |
| Steal the return value's refcount in `SQVM::Return()` via `_Swap(*dest, callee_slot)` instead of `*dest = callee_slot` followed by `LeaveFrame()` nulling/releasing that same slot, eliminating one addref+release pair per non-trivial return (mirrors the `_Swap` already used by `_OP_GET` / `_OP_GETK`) | targeted refcount-equivalence proof held: `samples/class.nut` and `samples/generators.nut` were byte-identical and all six workload checksums matched; a deterministic callgrind pass showed a consistent small **`-0.05%` / `-0.10%` / `-0.13%` / `-0.11%` / `-0.05%` / `-0.06%`** (≈`-0.09%` aggregate) instruction-count reduction | rejected under PGO+LTO despite the deterministic Ir win: pinned source A/B at `--run-repeat 80` was pure noise (control `356.211 ms` versus candidate `357.962 ms` order-1, then candidate `354.576 ms` versus control `356.619 ms` order-2 — `+0.57%` / `-0.49%` split), but a fresh pinned PGO+LTO retry (candidate retrained with `train-pgo.sh`, `-DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON`) lost cleanly in both orders: candidate `45.942 / 53.932 / 46.977 / 43.653 / 49.241 / 49.234 ms` (`288.979 ms`) versus retained head `44.903 / 49.180 / 46.504 / 42.477 / 47.775 / 53.250 ms` (`284.089 ms`, `-1.72%`), and reverse candidate `45.839 / 49.475 / 46.831 / 43.525 / 51.198 / 55.286 ms` (`292.154 ms`) versus retained head `46.715 / 49.578 / 46.360 / 44.596 / 48.134 / 49.035 ms` (`284.418 ms`, `-2.72%`) | rolled back; even an analytically refcount-equivalent change with a deterministic instruction-count reduction can lose under PGO+LTO because PGO had already shaped the return path, so source-Ir no longer predicts retained-head wall-clock for changes that interact with PGO's code shaping |
| Direct-threaded dispatch (GCC labels-as-values): replace the `switch(_i_.op)` interpreter dispatch with a 64-entry `sq_dispatch[]` label-address table and `goto *sq_dispatch[_i_.op]`, converting all 62 opcode `case` labels to ordinary labels (including the `_OP_TAILCALL` → `_OP_CALL` fallthrough); the table is declared once per `Execute()` entry, outside the dispatch loop and non-static, so no init-guard ever runs on the dispatch path | correctness stayed clean across two implementation variants: six-workload checksums matched (`2019808 / 325170 / 580946 / 593415 / 2030324 / 308206`), and `samples/class.nut` / `samples/generators.nut` were byte-identical to HEAD; the host is heavily loaded (`load avg ~34/24`), so a deterministic callgrind Ir baseline was added as an extra pre-filter and showed a consistent **`-3.61%`** aggregate instruction-count reduction across the six workloads (`-2.92%` / `-3.26%` / `-3.58%` / `-3.59%` / `-3.93%` / `-4.28%`) | a first attempt placed the table `static` inside the `for(;;)` loop and regressed wall-clock `+4%` / `+5%` in both orders because GCC's static-init guard then ran every dispatch; moving the table outside the loop (non-static) fixed that and produced a clean source-level win — two pinned source A/B pairs both improved: control `59.696 / 61.996 / 58.964 / 53.861 / 59.006 / 63.678 ms` (`357.201 ms`) versus candidate `58.485 / 60.159 / 59.289 / 53.317 / 58.481 / 63.526 ms` (`353.257 ms`, `+1.11%`), then candidate `60.896 / 61.033 / 59.276 / 54.233 / 58.811 / 62.340 ms` (`356.589 ms`) versus reverse control `60.580 / 62.822 / 59.892 / 54.469 / 59.568 / 70.767 ms` (`368.098 ms`, `+3.13%`) — but a fresh pinned PGO+LTO retry (candidate retrained with `train-pgo.sh`, `-DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON`, `run-repeat 12`) lost cleanly in both orders: candidate `47.399 / 50.563 / 46.988 / 43.568 / 56.779 / 51.292 ms` (`296.589 ms`) versus retained head `44.986 / 48.840 / 46.986 / 42.105 / 48.425 / 49.528 ms` (`280.870 ms`, `-5.61%`), and reverse candidate `48.562 / 50.574 / 46.141 / 46.618 / 54.663 / 50.575 ms` (`297.133 ms`) versus retained head `45.760 / 48.497 / 47.300 / 42.460 / 48.211 / 54.230 ms` (`286.458 ms`, `-3.72%`), with the largest, most consistent loss on dispatch-heavy `scenario_tick_flow` (`+13%` / `+13%`) | rolled back; source win did not survive PGO — profile-guided optimization already makes `switch` dispatch branch-predict and lay out better than a computed-goto table load on this CPU, so direct threading is ruled out for the PGO build (and the earlier `static`-in-loop variant is a known trap to avoid) |
...
This was the retained head before the `_OP_PREPCALLK` cached-delegate `clo` copy elimination promotion (the `get_slice_params()` no-copy head). Its authoritative six-workload total was `282.493 ms` on the current (loaded) host; the live pinned PGO+LTO A/B (`+1.07%` / `+0.44%` candidate wins in both orders) is the authoritative comparison.
...
### Promoted: avoid the refcount copy in the `_OP_PREPCALLK` cached-delegate path (current retained head)

The `_OP_PREPCALLK` cached-delegate fast path (the one that serves `len` / `tointeger` / `tofloat` / `tostring` / `slice` on tables/arrays/strings/numbers) used to begin every hit with `SQObjectPtr clo = cached;`, copying the cached native closure purely so the fallback `CallNative(_nativeclosure(clo), ...)` could read the callee back out of `clo`. `cached` is already in scope and is already what the intrinsic `TryFastCallNative(_nativeclosure(cached), ..., clo, ...)` call passes as the callee, so the copy was redundant: `clo` now starts null and is used only as the call's `retval` out-parameter, and the fallback passes `_nativeclosure(cached)` directly. This removes one addref+release per cached-delegate call (called millions of times across every workload) and is PGO-proof (the copy is through a local whose address escapes as the `retval` reference, so PGO cannot elide it).

| Baseline used | Result | Overall |
| --- | --- | ---: |
| retained PGO+LTO head `build-pgo-lto-gsp` (frozen `282.493 ms`) | six-workload checksum match, `samples/class.nut` / `samples/generators.nut` byte-identical, callgrind Ir **`-1.206%`** aggregate with every workload improving (`-0.717%` / `-1.245%` / `-1.162%` / `-1.762%` / `-1.287%` / `-1.122%`), fresh PGO+LTO retrain, and clean pinned PGO+LTO A/B aggregate wins in both orders: candidate `46.955 / 48.472 / 46.322 / 41.894 / 47.959 / 49.901 ms` (`281.503 ms`) versus live control `45.096 / 52.907 / 45.722 / 43.120 / 48.493 / 49.237 ms` (`284.575 ms`), then reverse candidate `45.971 / 49.446 / 46.937 / 43.689 / 48.813 / 50.740 ms` (`285.596 ms`) versus live control `47.132 / 50.345 / 47.294 / 42.790 / 49.989 / 49.321 ms` (`286.871 ms`); `world_map_graph` improved `-8.4%` and `session_context_flow` `-2.8%` in the first pair | `+1.07%` / `+0.44%` (live A/B, both orders) |
