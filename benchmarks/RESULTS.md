...
| --- | --- | --- | --- |
| Direct-threaded dispatch (GCC labels-as-values): replace the `switch(_i_.op)` interpreter dispatch with a static 64-entry `sq_dispatch[]` label-address table and `goto *sq_dispatch[_i_.op]`, converting all 62 opcode `case` labels to ordinary labels (including the `_OP_TAILCALL` → `_OP_CALL` fallthrough) | correctness stayed clean: six-workload checksums matched (`2019808 / 325170 / 580946 / 593415 / 2030324 / 308206`), and `samples/class.nut` and `samples/generators.nut` were byte-identical to HEAD; the host is currently heavily loaded (`load avg ~34/24`), so a deterministic callgrind Ir baseline was added as an extra pre-filter and showed a consistent **`-3.61%`** aggregate instruction-count reduction across the six workloads (`-2.92%` / `-3.26%` / `-3.58%` / `-3.59%` / `-3.93%` / `-4.28%`) | rejected during source smoke before a pinned PGO retry despite the Ir win: two pinned source A/B pairs both regressed overall because wall-clock rose while instruction count fell — first-order control `59.827 / 60.613 / 59.794 / 54.506 / 60.416 / 73.012 ms` (`368.168 ms`) beat candidate `68.085 / 66.205 / 61.194 / 59.882 / 60.614 / 66.906 ms` (`382.886 ms`, `-4.00%`), and reverse-order candidate `69.518 / 64.544 / 62.388 / 56.857 / 60.923 / 64.962 ms` (`379.192 ms`) again lost to control `59.211 / 61.529 / 58.647 / 57.336 / 60.384 / 64.648 ms` (`361.755 ms`, `-4.83%`), with the largest, most consistent loss on call/allocation-heavy `registry_catalog` (`+15%`) | rolled back; Ir is necessary but not sufficient — single dispatch-site direct threading trades switch overhead for an extra table-load/indirect-jump dependency whose frontend/branch cost (not captured by Ir) dominates on a loaded machine, so a full per-handler `NEXT()` variant was not pursued |
